...
def objective_function_control(params, fixed_materials, return_details=False):
    ...
    Loss = Σ|λ - mean(λ)|² where λ are eigenvalues of G1 = -G - 0.5i*I
    ...
    try:
        ...
        if np.any(np.isnan(eigvals)) or np.any(np.isinf(eigvals)):
            if return_details:
                return 1e10, eigvals, np.zeros(3), np.zeros(3), G, G1, 1e10, 0.0
            return 1e10
        ...
        if return_details:
            ...
            return loss, eigvals, real_parts, imag_parts, G, G1, spread, penalty_imag
        return loss
...
def optimize_exceptional_point(maxiter_de, maxiter_lbfgsb, seed, verbose=True):
    ...
    # Control_N DE settings (workers=1 to avoid nested multiprocessing)
    result_de = differential_evolution(
        ...
        tol=1e-12,
        updating='deferred',
        workers=1,  # Single worker per seed (outer parallelization handles seeds)
        polish=False,
        seed=seed,
        disp=False,
