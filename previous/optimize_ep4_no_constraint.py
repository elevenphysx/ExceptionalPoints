"""
Exceptional Point (EP4) Optimizer - NO CONSTRAINT VERSION
Two-stage optimization: Differential Evolution + L-BFGS-B
NO |Im(λ)| >= 5.0 constraint (penalty_weight = 0, only minimize variance)
"""
...
def _objective_wrapper_for_de(params):
    ...
    NO CONSTRAINT: penalty_weight=0
    """
    return objective_function_control(params, FIXED_MATERIALS, GreenFun, build_layers_func=build_layers_ep4, penalty_weight=0)
...
def optimize_exceptional_point(maxiter_de, maxiter_lbfgsb, seed, n_workers, verbose=True):
    ...
    output_dir = os.path.join('results', f'ep4_noconstraint_w{n_workers}_s{seed}_DE{maxiter_de}_LB{maxiter_lbfgsb}')
    os.makedirs(output_dir, exist_ok=True)

    if verbose:
        print(f"--> [Seed {seed}] Output directory: {output_dir}")
        print("=" * 70)
        print("WARNING: Constraint |Im(λ)| >= 5.0 is DISABLED")
        print("Penalty weight set to 0 (only minimize variance)")
        print("=" * 70)
    ...
    # Define objective function wrapper with penalty_weight=0 (NO CONSTRAINT)
    def objective_no_constraint(params, return_details=False):
        return objective_function_control(
            params, fixed_materials, GreenFun,
    ...
    def callback_de(xk, convergence=None):
        ...
        loss_current = objective_no_constraint(xk)
        ...
        if iteration_count[0] % 100 == 0 or iteration_count[0] == 1:
            loss, eigvals, real_parts, imag_parts, _, _, spread, pen_im = objective_no_constraint(
                xk, return_details=True
            )
    ...
    # --- Final Output ---
    loss, eigvals, real_parts, imag_parts, G, G_shifted, spread, pen_im = objective_no_constraint(
        final_x, return_details=True
    )
    ...
    with open(params_txt_path, 'w', encoding='utf-8') as f:
        f.write("=" * 70 + "\n")
        f.write(f"Exceptional Point (EP4) Parameters - Seed {seed}\n")
        f.write("=" * 70 + "\n\n")
