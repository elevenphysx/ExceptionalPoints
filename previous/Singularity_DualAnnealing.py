...
def optimize_exceptional_point(maxiter_de=100, maxiter_nm=500, maxiter_powell=500, seed=812):
    ...
    def da_callback(x, f, context):
        ...
        # Every 10 iterations (but not 100): simple loss output and record history
            ...
            msg = f"dual_annealing step {iteration_count[0]}/{maxiter_de} ({progress_pct:.1f}%): f(x)= {loss:.15f} (degeneracy={loss_degeneracy:.6e}, penalty_re={penalty_real:.6e}, penalty_im={penalty_imag:.6e})"
            pbar_da.write(msg)
            log_file.write(msg + "\n")
            log_file.flush()
