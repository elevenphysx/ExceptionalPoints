"""
Exceptional Point (EP4) Optimizer using CMA-ES
CMA-ES (Covariance Matrix Adaptation Evolution Strategy) for 4 resonant layers
"""
...
def optimize_exceptional_point(maxiter_cma, seed, verbose=True):
    ...
    output_dir = os.path.join('results', f'ep4_cma_s{seed}_iter{maxiter_cma}')
    ...
    def objective_cma(params):
        # CMA-ES minimizes, our function already returns loss to minimize
        loss = objective_function_control(params, fixed_materials, GreenFun, build_layers_func=build_layers_ep4)
        return loss
    ...
    def callback_cma(es):
        ...
        if iteration_count[0] % 10 == 0 or iteration_count[0] == 1:
            ...
            loss, eigvals, real_parts, imag_parts, _, _, spread, pen_im = objective_function_control(
                x_to_evaluate, fixed_materials, GreenFun, return_details=True, build_layers_func=build_layers_ep4
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Exceptional Point (EP4) Optimizer - CMA-ES Algorithm')
    ...
    print("=" * 70)
    print(f"EP4 Optimizer - CMA-ES (seed={args.seed})")
