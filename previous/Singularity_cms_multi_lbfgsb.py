"""
Exceptional Point Finder (Parallel CMA-ES + L-BFGS-B Strategy)
...
Strategy:
    Stage 1: Global Search using CMA-ES (soft constraints).
    Stage 2: Local Refinement using L-BFGS-B (hard box constraints).
...
from config import (FIXED_MATERIALS, BOUNDS, C0_FIXED, DEFAULT_THRESHOLD,
                    ...
                    DEFAULT_LBFGSB_ITERATIONS, DEFAULT_SEEDS,
...
def optimize_exceptional_point(maxiter_cma, maxiter_lbfgsb, seed, threshold, penalty_weight, verbose=True):
    np.random.seed(seed)

    output_dir = os.path.join('results', f'CMA_LBFGSB_s{seed}_thr{threshold:.1f}_pw{penalty_weight:.1f}')
    ...
    while not es.stop():
        iteration_count[0] += 1
        solutions = es.ask()
        fitnesses = [objective_function(x, fixed_materials, threshold=threshold, penalty_weight=penalty_weight) for x in solutions]
    ...
    # --- Phase 2: L-BFGS-B ---
    log_print("\n" + "=" * 70, console=False)
    log_print("Phase 2: L-BFGS-B Refinement (Hard Box Constraints)", console=False)
    log_print("=" * 70, console=False)
    ...
    try:
        res_lbfgsb = minimize(obj_wrapper, result_cma_x,
                             method='L-BFGS-B',
                             bounds=bounds,
                             callback=lbfgsb_callback,
                             options={'maxiter': maxiter_lbfgsb, 'ftol': 1e-12, 'gtol': 1e-12})
    ...
    for i, (val, (low, high)) in enumerate(zip(final_x, bounds)):
        status = "OK" if low <= val <= high else "VIOLATED"
        output += f"  param[{i}] = {val:.6f} in [{low}, {high}] -> {status}\n"
    ...
    plot_optimization_history(history, output_dir, seed, 'CMA-ES + L-BFGS-B')
...
def run_single_seed(args):
    seed, max_cma, max_lbfgsb, threshold, penalty_weight = args
    try:
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Exceptional Point Finder (Parallel CMA-ES + L-BFGS-B)')
    ...
    parser.add_argument('-s', '--seeds', type=int, nargs='+', default=DEFAULT_SEEDS, help='List of seeds to run')
    ...
    print("=" * 70)
    print(f"Starting Parallel Optimization (CMA-ES + L-BFGS-B)")
