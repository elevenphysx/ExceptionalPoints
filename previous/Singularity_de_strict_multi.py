...
def objective_function(params, fixed_materials, return_details=False, threshold=5.0, penalty_weight=100.0, bounds=None):
    """
    Wrapper with DE-specific penalty (discontinuous but sharper for gradient-free optimization)
    """
    ...
    try:
        ...
        G1 = -G  # Temporarily commented out: - 1j * 0.5 * np.eye(G.shape[0], dtype=complex)
        eigvals = np.linalg.eigvals(G1)

        if np.any(np.isnan(eigvals)) or np.any(np.isinf(eigvals)):
            if return_details:
                return 1e10, eigvals, np.zeros(3), np.zeros(3), G, G1, 1e10, (0.0, 0.0)
            return 1e10
        ...
        # No magnitude constraint - removed threshold penalty
        penalty_real = 0.0
        penalty_imag = 0.0

        # Soft bounds penalty (optional)
        penalty_bounds = 0.0
        if bounds is not None:
            for p, (low, high) in zip(params, bounds):
                if p < low:
                    penalty_bounds += 1000 * (low - p)**2
                elif p > high:
                    penalty_bounds += 1000 * (p - high)**2
...
def optimize_exceptional_point(maxiter_de, maxiter_powell, seed, threshold, penalty_weight, verbose=True):
    np.random.seed(seed)

    output_dir = os.path.join('results', f'DE_Strict_s{seed}_thr{threshold:.1f}_pw{penalty_weight:.1f}')
    ...
    def callback_de(xk, convergence=None):
        nonlocal best_loss_so_far, best_solution_so_far  # <--- FIX IS HERE
        
        iteration_count[0] += 1
        pbar_de.update(1)
        
        # DE works on populations, xk is the best candidate of current generation
        loss_current = objective_function(xk, fixed_materials, bounds=bounds, threshold=threshold, penalty_weight=penalty_weight)
        ...
        if iteration_count[0] % 10 == 0 or iteration_count[0] == 1:
            ...
            msg = f"DE Iter {iteration_count[0]}: Loss={loss:.6e} (Deg={loss_deg:.6e})"
        ...
        # Stop if maxiter reached (DE acts slightly differently with maxiter)
        if iteration_count[0] >= maxiter_de:
            return True 

    def obj_wrapper(p):
        return objective_function(p, fixed_materials, bounds=bounds, threshold=threshold, penalty_weight=penalty_weight)

    # Note: DE natively enforces bounds, so xk will always be valid.
    ...
    # Handle cases where DE might finish without updating best_solution_so_far (rare, but possible if initial pop is bad)
    if best_solution_so_far is None:
        best_solution_so_far = result_de.x
        
    result_de_x = best_solution_so_far # Use the best seen, not just the final state
    
    # Clip just to be 100% safe before Powell
    lower = np.array([b[0] for b in bounds])
    upper = np.array([b[1] for b in bounds])
    result_de_x = np.clip(result_de_x, lower, upper)

    # --- Phase 2: Powell Refinement (Strict Bounds) ---
    log_print("\n" + "=" * 70, console=False)
    ...
    try:
        # Using Powell with bounds to fix the negative thickness issue
        res_pw = minimize(obj_wrapper, result_de_x, method='Powell', 
                         bounds=bounds, 
    ...
    output += f"  Min|Re| = {np.min(np.abs(real_parts)):.4f}\n"
    output += f"  Min|Im| = {np.min(np.abs(imag_parts)):.4f}\n"
    ...
    plot_optimization_history(history, output_dir, seed, 'DE + Powell')
...
def run_single_seed(args):
    seed, max_de, max_pw, threshold, penalty_weight = args
    try:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Exceptional Point Finder (DE + Strict Powell)')
    ...
    print("=" * 70)
    print(f"Starting Parallel Optimization (DE + Strict Powell)")
    print(f"Algorithm: DE (Global + Strict) -> Powell (Local + Strict)")
    print("=" * 70)
